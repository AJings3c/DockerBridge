        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("DockerBridge has been initialized. If you want to run setup again, please delete the database.");
                }
            ...
            try {
                ...
                if (user) {
                    ...
                    callback({
                        ...
                        permissions: permissionsForRole(normalizeUserRole(user.role)),
                    });
                } else {
        ...
        socket.on("login", async (data, callback) => {
            ...
            if (user) {
                ...
                if (user.twofa_status && data.token) {
                    ...
                    // @ts-ignore
                    const verify = notp.totp.verify(data.token, user.twofa_secret, {
                        time: 30,
                        window: 1,
                    });
